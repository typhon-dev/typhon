...
- Frontend:
  ...
  - Parsing: Conversion of tokens into an AST using the ASTParser trait
  - Type checking: Static type analysis of the AST
- Middle-end: Analysis and transformation of the typed AST
- Backend: Code generation and LLVM integration

The use of the ASTParser trait in the frontend allows for a more flexible and maintainable parsing system, with each AST node type implementing its own parsing logic while adhering to a common interface.
